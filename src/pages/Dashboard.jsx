 ...
 */
const NavBar = memo(function NavBar({ selectedExam, onExamChange, entryCount, onToggleHistory, user, onLogout }) {
...
const MotivationalSection = memo(function MotivationalSection({ selectedExam, customQuote }) {
  ...
  return (
    <section aria-label="Motivational encouragement" className="fade-in-up rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-1)] p-6">
      ...
      <div className="space-y-4">
        {motivationalContent && [(customQuote || (motivationalContent[selectedExam] && motivationalContent[selectedExam][0]))].map((item, idx) => item && (
          <article
            key={idx}
            ...
            <>
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <div className="space-y-6">
                  {lastAnalysis.analysis && <StressTriggerSection triggers={lastAnalysis.analysis.stressTriggers} />}
                  {lastAnalysis.analysis && <EmotionalPatternsSection patterns={lastAnalysis.analysis.emotionalPatterns} />}
                </div>

                {/* Right Column — Support */}
                <div className="space-y-6">
                  {lastAnalysis.analysis && <CopingStrategiesSection strategies={lastAnalysis.analysis.copingStrategies} />}
                  {lastAnalysis.analysis && <MindfulnessSection exercises={lastAnalysis.analysis.mindfulnessExercises} />}
                  {lastAnalysis.analysis && <MotivationalSection selectedExam={selectedExam} customQuote={lastAnalysis.analysis.motivationalQuote} />}
                </div>
              </div>
            </>
          )}
